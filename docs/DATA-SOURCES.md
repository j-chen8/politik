...
> Pendant** steht 1:1-strukturiert in **§0-B** direkt darunter — bei einem Berlin-Refresh
> dort nachsehen (eigene Skripte, eigene Stränge).
...
## 0-B. »update«-Runbook BERLIN (PARDOK) — Trigger: „update berlin" / Berlin-Refresh

> **Vertrag:** Spiegelt §0 für die Berlin-PARDOK-Pipeline. Gratis-Schritte autonom.
> **⚠️ Anders als BT-§0:** LLM-Schritte **IMMER mit Kosten-OK** — KEINE stehende
> Freigabe (User-Regel: kein Limit, immer fragen, [[feedback_kosten_ok_vor_api]]).
...
**⚠️ Kosten-Realität (Vermerk):** Pre-Flight-Schätzungen laufen **~1,7× zu niedrig** (Cache-Write-Tokens unterschätzt). 2026-06-17: Schätzung $3 → real ~$5. Bei der Vorlage einkalkulieren.

**⚠️ Verlorene `_`-Skripte:** `_post-process-vote-outcomes.ts` (Vote-Outcome-Heuristik) ist bei einem Track-Reset verloren (wie früher `check-data-freshness.ts`). Display kollabiert `annahme_geaendert`→„Angenommen", daher unkritisch; bei Bedarf neu bauen. Der Leak-Cleaner wurde 2026-06-17 rekonstruiert (jetzt ohne `_`-Präfix → committet).

**Definition of Done (Berlin):** Gratis-Gaps geschlossen · alle Batches applied · Leak-Cleaner + Prune + Reden-Erben gelaufen · 0 FK-Orphans/Phantom-Votes · Neutralitäts-Spotcheck bestanden · ehrlicher Bericht inkl. Echt-Kosten + Caveats (z.B. künftige Sitzung ohne Protokoll).
