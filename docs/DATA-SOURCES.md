...
- **⚠️ Re-Fetch-Fix 2026-06-14:** Bundestag veröffentlicht erst eine VORLÄUFIGE Protokoll-Fassung, Tage/Wochen später die ENDGÜLTIGE (nachgereichte „zu Protokoll gegebene" Reden + späte TOPs) unter gleichem Dateinamen, aber NEUER Blob-ID. `fetch-plenar-xmls.ts` lud früher nur fehlende Dateinamen → blieb auf der vorläufigen Fassung hängen (15 Sitzungen stale, 565 Reden fehlten, z. B. Warken/Medizinregister Sitzung 80). Jetzt vergleicht es Blob-IDs (Sidecar `data/plenarprotokolle_xml/_blob-ids.json`) und **re-fetcht geänderte Sitzungen**; es meldet am Ende die „aktualisierten" Sitzungen → für die `extract-all-speeches.ts` (idempotenter UPSERT per `rede_id` — fügt nur neue Reden, lässt Mediathek/Analysen unangetastet) + Analyse + `backfill-speech-summaries-from-v2.ts` re-laufen lassen.
...
- **⚠️ Pflicht-Schritt danach (v2→speech_summaries-Kopie, $0):** Die UI liest `speech_summaries` — neue Reden bleiben dort LEER, bis die `zusammenfassung` aus `speech_analyses_v2` kopiert ist (segment-geordnet je `rede_id` joinen, `model='backfill-from-v2-<datum>'`). Am 2026-06-11 entdeckt: Sitzungen 76–80 waren analysiert, aber 737 Summaries leer → Reden fehlten still auf Themen-Blatt/Profilen. Skript: `backfill-speech-summaries-from-v2.ts` (seit 2026-06-12, idempotent).
...
- **Discovery (autoritativ):** DIP-API `GET /api/v1/drucksache?f.wahlperiode=21&f.zuordnung=BT` (Cursor-Pagination) = vollständige Liste ALLER WP21-Bundestag-Drucksachen (`21/NNNN`). >10 MB werden geskippt, 404 = (noch) kein PDF.
- **DB-Watermark:** `SELECT COUNT(*), MAX(drucksache_nr) FROM drucksache_texts;`
- **⚠️ Discovery-Fix 2026-06-13:** Vorher zog `download-missing-drucksachen.sh` nur DS aus `activities WHERE dokumentart='Drucksache'` → **aktivitäts-getrieben, strukturell unvollständig** (verfehlt Regierungs-Gesetzentwürfe, Ausschuss-Beschlussempfehlungen, Unterrichtungen — z. B. 21/5922 Medizinregister fehlte komplett). Ersetzt durch DIP-Voll-Enumeration. **NICHT mehr** `download-missing-drucksachen.sh` (löchrig) noch `download-drucksachen.sh` (toter `/home/jk/`-Pfad + Cookie) verwenden.
- **Pipeline ($0):** `npx tsx scripts/sync-drucksachen-pdfs.ts` (DIP-Voll-Enumeration → lädt alle fehlenden PDFs; schreibt Manifest `data/drucksachen/_dip-bt-manifest.tsv`; idempotent, `--dry-run` zeigt nur die Lücke) → `npx tsx scripts/extract-drucksache-texts.ts` (PDF→`drucksache_texts`) → **`npx tsx scripts/classify-drucksachen.ts`** (setzt `batch_class`, Regex/Activity-Vorrang) → **`npx tsx scripts/label-administrativ-drucksachen.ts`** (Regex-Labels für `administrativ`-Klasse) → **`npx tsx scripts/extract-drucksache-publication-date.ts`** (Datum aus PDF-Header → `publication_date`) — alle idempotent
...
- **⚠️ NICHT** `scripts/download-drucksachen.sh` (toter `/home/jk/`-Pfad + Cookie) noch `download-missing-drucksachen.sh` (activities-getrieben, unvollständig — siehe Discovery-Fix oben) nutzen; beide durch `sync-drucksachen-pdfs.ts` ersetzt
