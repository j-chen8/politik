/**
 * Re-Run der 209 heavy-truncated Drucksachen mit tiered Length-Prompts.
 *
 * Strategie:
 *  - Selektiere alle DS mit tokens_estimate > 2× cap (= heavy truncated im v1-Lauf)
 *  - Bestimme Tier (standard / long / massive) anhand tokens_estimate + pages
 *  - Nutze tier-spezifisches System-Prompt mit gewünschten Längen
 *  - Effektiv kein Truncate: Cap = 180K Tokens (lässt 20K für Prompt/Output bei 200K Haiku-Standard-Context)
 *  - Submit via Batch API
 *  - Auf Ingest: UPDATE existing records, setze prompt_version='v1.1'
 *
 *   --dry-run        Vorschau (zeige Klassifikation + Cost-Estimate)
 *   --submit         Reicht den Batch ein
 *   --poll <id>      Pollt Status + Schreibt Ergebnisse
 *   --resume         Übernimmt offenen Batch aus drucksache_batch_runs
 *   --limit N        Nur erste N Records (für Smoke-Test)
 */
...
// Effektiv kein Truncate für 209 Re-Run: 100K unserer Tokens
// Data-Dumps tokenisieren extrem dicht (Ratio 1.7×). 100K our → max ~170K real
// für die dichtesten DS, lässt 30K Buffer für System+Output bei 200K Context.
const RERUN_CAP_TOKENS = 100_000;
const MAX_OUTPUT_TIERS: Record<LengthTier, number> = {
  standard: 2000,   // ~1200 Wörter Headroom für JSON
  long:     5000,
  massive:  10000,
};
...
async function submitBatch(rows: Row[]) {
  ...
  if (DRY_RUN) {
    console.log("--dry-run: kein API-Call. Beispiel-Request (für ersten):");
    const first = JSON.stringify(requests[0], null, 2);
    console.log(first.slice(0, 1800) + "\n…");
    return;
  }
  ...
  db.prepare(
    ...
    "all", batch.processing_status, "tiered-rerun-v1.1"
  );

  console.log(`\nPolling:  npx tsx scripts/rerun-truncated-drucksachen.ts --poll ${batch.id}`);
}
...
async function pollAndIngest(batchId: string) {
  ...
  console.log("\n📥 Lade Ergebnisse …");
  // UPDATE bestehende Records statt INSERT, prompt_version='v1.1'
...
async function main() {
  ...
  if (RESUME) {
    const open = db.prepare(`SELECT batch_id FROM drucksache_batch_runs WHERE notes='tiered-rerun-v1.1' AND status != 'ended' ORDER BY submitted_at DESC LIMIT 1`).get() as { batch_id: string } | undefined;
    if (!open) { console.log("Kein offener Tiered-Rerun-Batch."); return; }
  ...
  console.log(`📋 ${rows.length} heavy-truncated DS zum Re-Run`);
  ...
  const inCost = (tokSum / 1e6) * 0.50;   // Haiku Batch Input
  const outCost = (outSum / 1e6) * 2.50;  // Haiku Batch Output (worst case if all maxed)
