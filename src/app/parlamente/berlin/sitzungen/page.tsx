  ...
  return (
    <div className="page-wash">
      <div className="w-full page-shell">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-zinc-200/70 dark:bg-zinc-700/70 rounded-2xl overflow-hidden border border-border mb-10 fade-in-up fade-in-up-2">
        ...
        <div className="space-y-8 fade-in-up fade-in-up-3">
          {groups.map((g) => (
            <div key={g.year}>
              <div className="text-[12px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-3 num">{g.year}</div>
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div className="bg-card px-4 py-4">
      ...
      <div className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-tight mt-0.5">{label}</div>
    </div>
  );
}
...
function SitzungCard({ s }: { s: BerlinSitzungListEntry }) {
  ...
  return (
    <Link href={`/parlamente/berlin/sitzung/${s.nr}`} className="block border border-border rounded-2xl bg-card px-5 py-4 hover:bg-zinc-50/60 dark:hover:bg-zinc-800/60 hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors group">
      <div className="flex items-center gap-4">
        <div className="shrink-0 w-14 text-center">
          <div className="text-2xl font-semibold tracking-[-0.02em] num leading-none">{s.nr}</div>
          <div className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mt-0.5">Sitzung</div>
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-[14px] font-medium text-zinc-950 dark:text-zinc-50 group-hover:text-zinc-700 dark:group-hover:text-zinc-300 transition-colors">
            {datum ?? <span className="text-zinc-400 dark:text-zinc-500">Datum nicht erfasst</span>}
          </div>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5 text-[12px] text-zinc-500 dark:text-zinc-400">
            <span className="inline-flex items-center gap-1"><ListTree className="w-3.5 h-3.5 text-zinc-300 dark:text-zinc-600" strokeWidth={2} /><span className="num">{s.tops}</span> Tagesordnungspunkte</span>
            <span className="inline-flex items-center gap-1"><Mic className="w-3.5 h-3.5 text-zinc-300 dark:text-zinc-600" strokeWidth={2} /><span className="num">{s.reden}</span> Wortbeiträge</span>
            {s.votes > 0 && <span className="inline-flex items-center gap-1"><Vote className="w-3.5 h-3.5 text-zinc-300 dark:text-zinc-600" strokeWidth={2} /><span className="num">{s.votes}</span> Abstimmungen</span>}
          </div>
        </div>
        <ArrowRight className="w-4 h-4 text-zinc-400 dark:text-zinc-500 group-hover:text-zinc-900 dark:group-hover:text-zinc-100 group-hover:translate-x-0.5 transition-all shrink-0" strokeWidth={2.25} />
      </div>
    </Link>
  );
}
