...
/**
 * Berlin-Detailsuche — gespiegelt zur Bundestag-/suche/detail, scope="berlin".
 * Typ-Wahl (Drucksachen/Reden/Personen), Sortierung (Datum/Relevanz),
 * Drucksachen-Typ-Filter, Synonym-Toggle. Ergebnisse über SearchFullList.
 */
...
function DetailInner() {
  ...
  function update(patch: Record<string, string | null>) {
    ...
    next.set("page", "1");
    router.push(`/parlamente/berlin/suche/detail?${next.toString()}`);
  }
    ...
    <div className="page-wash min-h-screen">
      <div className="max-w-3xl mx-auto px-5 pt-24 pb-24 fade-in-up">
        ...
        <p className="text-[13px] text-zinc-500 dark:text-zinc-400 mb-6">
          Abgeordnetenhaus Berlin · nach Typ filtern, Sortierung wählen, verwandte Begriffe optional einbeziehen.
        </p>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-8 text-[12px]">
          ...
          {type === "drucksachen" && (
            <label className="flex items-center gap-1.5">
              ...
              <select className={selectCls} value={klasse ?? ""} onChange={(e) => update({ klasse: e.target.value || null })}>
                {KLASSEN.map((k) => (
        ...
        {q.trim().length >= 2 ? (
          <SearchFullList query={q} type={type} page={page} expand={expand} sort={sort} klasse={klasse} embedded scope="berlin" />
        ) : (
...
export default function BerlinDetailSuchePage() {
  return (
