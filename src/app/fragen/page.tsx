  ...
  return (
    <div className="max-w-3xl mx-auto px-5 pt-24 pb-24">
      <div className="flex items-center gap-2 mb-1">
        <MessageSquareQuote className="w-5 h-5 text-[#1a3e72] dark:text-[#8fb3e6]" strokeWidth={2} />
      ...
      <form action="/fragen" method="get" className="mb-6 space-y-2.5">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 border border-zinc-300 dark:border-zinc-600 rounded-lg px-3 py-2 text-[13.5px] focus:outline-none focus:border-[#1a3e72] dark:focus:border-[#8fb3e6]"
          />
          <button type="submit" className="px-4 py-2 rounded-lg bg-[#1a3e72] dark:bg-[#8fb3e6] text-white text-[13px] font-medium hover:bg-[#0f2a52] dark:hover:bg-[#b7d0f0] transition-colors">Suchen</button>
        </div>
        <div className="flex flex-wrap gap-2 items-center">
          <label className="text-[11.5px] text-zinc-500 dark:text-zinc-400">Partei</label>
          <select
            name="partei" defaultValue={partei ?? ""}
            className="border border-zinc-300 dark:border-zinc-600 rounded-lg px-2.5 py-1.5 text-[12.5px] bg-card focus:outline-none focus:border-[#1a3e72] dark:focus:border-[#8fb3e6]"
          >
          ...
          <label className="text-[11.5px] text-zinc-500 dark:text-zinc-400 ml-2">Sortierung</label>
          <select
          ...
          <button type="submit" className="px-3 py-1.5 rounded-lg border border-zinc-300 dark:border-zinc-600 text-zinc-700 dark:text-zinc-300 text-[12px] font-medium hover:border-zinc-400 dark:hover:border-zinc-500 transition-colors">Anwenden</button>
        </div>
      </form>
      ...
      <ul className="space-y-5">
        {items.map((qa) => (
          <li key={`${qa.drucksacheNr}-${qa.paarIndex}`} className="border border-border rounded-xl bg-card px-5 py-4">
            <div className="flex items-baseline gap-2 flex-wrap mb-1.5 text-[11.5px]">
              {qa.fragestellerPoliticianId ? (
                <Link href={`/politiker/${qa.fragestellerPoliticianId}`} className="font-medium text-zinc-950 dark:text-zinc-50 hover:text-[#1a3e72] dark:hover:text-[#8fb3e6] transition-colors">{qa.fragestellerName}</Link>
              ) : (
                <span className="font-medium text-zinc-700 dark:text-zinc-300">{qa.fragestellerName}</span>
              )}
              {qa.fragestellerParty && <span className="text-zinc-400 dark:text-zinc-500">{qa.fragestellerParty}</span>}
              <span className="text-zinc-200">·</span>
              <Link href={`/aktivitaeten/${qa.drucksacheNr.replace(/\//g, "-")}`} className="text-[#1a3e72] dark:text-[#8fb3e6] hover:text-[#0f2a52] dark:hover:text-[#b7d0f0] num transition-colors">{qa.drucksacheNr}</Link>
              {qa.datum && <span className="text-zinc-400 dark:text-zinc-500 num">{formatGermanDate(qa.datum)}</span>}
            </div>
            ...
            {qa.antwortText && (
              <details className="group">
                ...
                <p className="mt-1.5 text-[12.5px] text-zinc-600 dark:text-zinc-300 leading-relaxed whitespace-pre-line border-l-2 border-border pl-3">{qa.antwortText}</p>
              </details>
            )}
          </li>
        ))}
      ...
      {lastPage > 1 && (
        <div className="flex items-center justify-between mt-8 text-[12.5px]">
          {page > 1 ? <Link href={mkHref(page - 1)} className="text-[#1a3e72] dark:text-[#8fb3e6] hover:text-[#0f2a52] dark:hover:text-[#b7d0f0]">← Zurück</Link> : <span />}
          <span className="text-zinc-400 dark:text-zinc-500 num">Seite {page} / {lastPage}</span>
          {page < lastPage ? <Link href={mkHref(page + 1)} className="text-[#1a3e72] dark:text-[#8fb3e6] hover:text-[#0f2a52] dark:hover:text-[#b7d0f0]">Weiter →</Link> : <span />}
        </div>
      )}
    </div>
  );
