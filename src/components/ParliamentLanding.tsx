    ...
    <div className="page-wash">
      {topics ? (
        ...
        <section className="w-full max-w-6xl mx-auto px-5 pt-20 lg:pt-24 pb-10 fade-in-up">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 items-center">
            <div>
              <h1 className={`${headlineClassName} font-semibold tracking-[-0.04em] leading-[0.98] text-zinc-950 mb-3 text-balance`}>
                {headline}
              </h1>
              <p className="text-lg text-zinc-500 mb-2 leading-relaxed">{subtitle}</p>
              <div className="mb-7">
                <Link
                  href={methodikHref}
      ...
      <section className={`w-full max-w-6xl mx-auto px-5 ${footer ? "pb-12" : "pb-24"} fade-in-up fade-in-up-3`}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          {columns.map((col) => (
            <div key={col.title} className="border border-zinc-200/70 rounded-2xl bg-white px-5 py-5 flex flex-col">
              <h3 className={`text-[15px] font-semibold tracking-[-0.01em] text-zinc-950 ${col.subtitle ? "mb-1" : "mb-4"}`}>
                {col.title}
              </h3>
              {col.subtitle && (
                <p className="text-[12px] text-zinc-500 mb-4">{col.subtitle}</p>
              )}
              ...
              {col.footer && (
                <Link
                  ...
                  className={`mt-4 pt-3 border-t border-zinc-100 inline-flex items-center gap-1.5 text-[12.5px] font-medium ${accentClass} hover:gap-2 transition-all`}
                >
                  {col.footer.label}
