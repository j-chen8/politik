...
function ThemeCard({
  ...
  if (mode === "kompakt") {
    return (
      ...
      >
        <summary className="p-4 cursor-pointer hover:bg-zinc-50/50 transition-colors list-none">
          <div className="flex items-baseline gap-2 mb-1 flex-wrap">
            <span className="text-[11px] text-zinc-400 num">#{idx + 1}</span>
            <span
              className={`text-[10.5px] uppercase tracking-wider font-medium px-1.5 py-0.5 rounded ${
                isAmber ? "bg-amber-100 text-amber-900" : "bg-zinc-100 text-zinc-600"
              }`}
            >
              {meta.label}
            </span>
            <span className="text-[11px] text-zinc-400 num">{theme.timestamp_range}</span>
          </div>
          <h3 className="text-[15px] font-semibold text-zinc-950 leading-snug mb-2">
            {theme.title}
          </h3>

          {/* Frage IMMER zeigen wenn Schema-Reform-Daten vorhanden */}
          {theme.question_asked && (
            <div className={`text-[12.5px] leading-relaxed mb-2 ${isAmber ? "text-amber-900" : "text-zinc-600"}`}>
          ...
          {firstQuote && (
            <blockquote className="border-l-2 border-zinc-300 pl-3 text-[13px] text-zinc-700 italic leading-relaxed">
              ...
              <a
                ...
                className="not-italic text-[11px] text-zinc-500 hover:text-zinc-950 underline decoration-zinc-300 hover:decoration-zinc-950 num ml-2"
              >
          ...
          <div className="flex items-center justify-end mt-2 text-[11.5px] text-zinc-400 group-open:hidden">
            Mehr Details ↓
          </div>
          <div className="flex items-center justify-end mt-2 text-[11.5px] text-zinc-400 hidden group-open:flex">
            Weniger anzeigen ↑
          </div>
        </summary>
        ...
        <div className="px-4 pb-4 pt-1 border-t border-zinc-100 space-y-4">
          <p className="text-[12.5px] text-zinc-500 leading-relaxed">{theme.theme_description}</p>
          ...
          {(theme.question_intent || theme.match_reasoning) && (
            <div className={`rounded-lg p-3 text-[12.5px] leading-relaxed space-y-1.5 ${
              isAmber ? "bg-amber-50/50 border border-amber-200/60" : "bg-zinc-50 border border-zinc-200/60"
            }`}>
              {theme.question_intent && (
                <div>
                  <span className="text-[10.5px] uppercase tracking-wider font-semibold text-zinc-500 mr-2">
                    Erkenntnisinteresse
                  </span>
                  <span className="text-zinc-700">{theme.question_intent}</span>
                </div>
              )}
              ...
              {theme.match_reasoning && (
                <div>
                  ...
                  <span className="text-zinc-700">{theme.match_reasoning}</span>
                </div>
              )}
            </div>
          )}
          ...
          {!theme.match_reasoning && isAmber && theme.evasion_note && (
            <div className="bg-amber-50/50 border border-amber-200/60 rounded-lg p-3 text-[12.5px] text-zinc-700 leading-relaxed">
              <span className="text-[10.5px] uppercase tracking-wider font-semibold text-amber-900 mr-2">
          ...
          <div>
            ...
            <p className="text-[13.5px] text-zinc-700 leading-relaxed">{theme.position}</p>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {theme.quotes.slice(1).map((q, qi) => (
                  <blockquote
                    key={qi}
                    className="border-l-2 border-zinc-300 pl-3 text-[13px] text-zinc-700 italic leading-relaxed"
                  >
                    „{q.text}"
                    <div className="not-italic text-[11px] text-zinc-500 mt-1">
                      <a
                        href={youtubeUrlWithTimestamp(appearanceUrl, q.timestamp)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-zinc-950 underline decoration-zinc-300 hover:decoration-zinc-950 num"
                      >
                        Min. {q.timestamp}
                      </a>
  ...
  return (
    ...
    >
      <div className="p-5 border-b border-zinc-100">
        ...
        <h3 className="text-[16.5px] font-semibold text-zinc-950 leading-snug mb-1.5">
          {theme.title}
        </h3>
        <p className="text-[12.5px] text-zinc-500 leading-relaxed">{theme.theme_description}</p>
      </div>
      ...
      <div className="p-5 space-y-4">
        ...
        {(theme.question_asked || theme.question_intent || theme.match_reasoning) && (
          <div className={`rounded-lg p-3.5 space-y-2 text-[13px] leading-relaxed ${
            isAmber ? "bg-amber-50/50 border border-amber-200/60 text-zinc-700" : "bg-zinc-50 border border-zinc-200/60 text-zinc-700"
          }`}>
            {theme.question_asked && (
              <div>
                <span className={`text-[10.5px] uppercase tracking-wider font-semibold mr-2 ${
                  isAmber ? "text-amber-900" : "text-zinc-500"
                }`}>
                  Frage
                </span>
                {theme.question_asked}
        ...
        <div>
          ...
          <p className="text-[14px] text-zinc-700 leading-relaxed">{theme.position}</p>
        </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {theme.quotes.map((q, qi) => (
                <blockquote
                  key={qi}
                  className="border-l-2 border-zinc-300 pl-3 text-[13.5px] text-zinc-700 italic leading-relaxed"
                >
                  „{q.text}"
                  <div className="not-italic text-[11px] text-zinc-500 mt-1.5 flex flex-wrap items-baseline gap-x-2">
                    <a
                      href={youtubeUrlWithTimestamp(appearanceUrl, q.timestamp)}
                      target="_blank"
                      rel="noopener noreferrer"
